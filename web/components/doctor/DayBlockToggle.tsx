...
interface DayBlockToggleProps {
  ...
  /** Short weekday label ("Mon") for the compact day-off row under the grid. */
  label?: string;
}
 ...
 */
export function DayBlockToggle({ date, blockedOverrideId, label }: DayBlockToggleProps) {
  ...
  if (blockedOverrideId) {
    return (
      <Button
        ...
        variant="outline"
        size="sm"
        className="h-7 border-destructive/40 px-2 text-xs text-destructive"
        disabled={pending}
        onClick={unblock}
        aria-label={label ? `${label} is a day off — undo` : undefined}
      >
  ...
  return (
    <Button
      ...
      disabled={pending}
      onClick={block}
      aria-label={label ? `Take ${label} off` : undefined}
    >
      {label ? (
        label
      ) : (
