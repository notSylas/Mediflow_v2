...
| Logging | pino (`backend/core/logger.ts`) | |
...
### Slot engine (`backend/booking/slots.ts`, `backend/booking/availability.ts`)
...
### Video (`backend/video/video.ts`, `backend/video/call-window.ts`, `/api/appointments/[id]/video-token`)
...
### Consultation (`backend/consult/consult.ts`)
...
### AuthZ pattern (`backend/auth/api-auth.ts`)
