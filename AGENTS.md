...
- Out of v1 scope: complex dashboards, doctor discovery. AI scribe (transcript → draft SOAP note) is v1.5. Doctor self-registration (`/doctor/register`, gated by `DOCTOR_SIGNUP_CODE`) shipped 2026-08-25 — still single-doctor: it's a nicer way to create/replace THE canonical doctor, not a multi-doctor marketplace. Doctor RMP verification (`/doctor/verification`) + an admin review surface (`/admin`, `role: "admin"`, bootstrapped via `scripts/promote-admin.ts` — no self-service path) shipped 2026-08-26: a doctor submits registration details + documents, an admin manually cross-checks the public NMC registry and approves/rejects. Still single-doctor in effect — `getCanonicalDoctorProfile`'s oldest-row resolution is unchanged, so a second, verified doctor would still be inert on every patient-facing surface.
...
Next.js 16 (App Router) + TypeScript + Tailwind 4. Postgres via Drizzle (`backend/db/schema.ts`). Auth: Better Auth (`backend/auth/auth.ts`) — email OTP (logs to console in dev) + optional Google; users have a `role` field (`patient` | `doctor` | `admin`).
